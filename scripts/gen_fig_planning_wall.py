...
#                    most cells -> metrics.sr_pct; the F4 and F6 cells are the
#                    per-environment first-attempt rates recomputed from the
#                    full jsonl (F4 5/50 = 10, F6 19/50 = 38).  That run was
#                    restarted mid-way, so its own summary covers only the
#                    resumed segment (partial-segment counts 2/38 and 11/38);
#                    weighting every recorded episode instead gives 8/163 =
#                    4.9 and 45/164 = 27.4 on the same log.  The per-environment
#                    rates are the ones comparable with the single-rollout rows.
#                    (2026-09-20 correction: the F4 cell previously plotted the
#                    count 2 on the percentage axis.)
...
# F4_BARS only: GPT-5.1 -> gpt51_l3_f4/ (2/50).  The Gemma-4-26B bar was removed
# on 2026-09-20: no artifact exists in any reachable results tree, so the row
# carries no evidence.

TABLE_ROWS = [
    ("GPT-4o",          [100, 60, 72, 0, 66, 26], False),
    ("Gemini 3.1 Pro",  [100, 100, 100, 10, 100, 38], False),
...
N_WALL = 5  # in-context / F4-only probes above the search divider
PROBE_NAMES = {"GPT-5.1"}
...
def build():
    ...
    ys = [1.36, 1.225, 1.09, .955, .70, .44, .305]
    ...
    for i, ((name, vals, search), y) in enumerate(zip(rows, ys)):
        ...
        color = GREEN if i == 5 else INK if i == 6 else RED
